...
class CategoriesCubit extends Cubit<CategoriesState> {
  ...
  CategoriesCubit({CategoriesApiService? dataSource})
  ...
  Future<void> getCategories(String? accessToken) async {
    emit(GetCategoriesLoading());

    try {
      final response = await _dataSource.getCategories(accessToken);
  ...
  Future<void> getCategoryDetails(String? accessToken, int categoryId) async {
    emit(GetCategoryDetailsLoading());

    try {
      final response = await _dataSource.getCategoryDetails(
        accessToken,
        categoryId,
      );
      ...
      emit(GetCategoryDetailsSuccess(response));
    } catch (e) {
      emit(GetCategoryDetailsError(e.toString()));
    }
  }

  Future<void> getCategoryProducts(String? accessToken, int categoryId) async {
    ...
    try {
      ...
      emit(GetCategoryProductsSuccess(response));
    } catch (e) {
